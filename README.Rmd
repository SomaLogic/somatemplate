...
template for R packages developed by SomaLogic Operating Co., Inc. This package 
is designed to provide a cohesive visual identity for packages developed by 
SomaLogic, and we kindly request that you do not use it for your own personal 
package.
...
Once made public, `somatemplate` could be installed from GitHub as follows 
(please note that this method does not currently work):
...
is installed, and that the `DESCRIPTION` file contains the following line:
...
- Update `_pkgdown.yml` to reference `somatemplate`, as described in [Usage](#usage) above
